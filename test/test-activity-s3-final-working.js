...
async function testActivityS3FinalWorking() {
  try {
    console.log('🧪 [TEST ACTIVITY S3 FINAL WORKING] Prueba final de carga de actividades con imágenes a S3...\n');
    ...
    // 2. Crear cuenta de prueba con campos correctos
    console.log('\n2️⃣ Creando cuenta de prueba...');
    const accountData = {
      nombre: 'Cuenta de Prueba S3 Final',
      razonSocial: 'Razón Social de Prueba S3',
      address: 'Dirección de prueba S3',
      emailAdmin: 'admin.prueba.s3@test.com',
      nombreAdmin: 'Administrador Prueba S3'
    };
    ...
    const testAccount = accountResponse.data.data.account;
    console.log(`✅ Cuenta creada: ${testAccount.nombre} (${testAccount._id})`);

    // 3. Subir imagen a S3
    console.log('\n3️⃣ Subiendo imagen a S3...');
    const testImagePath = path.join(__dirname, 'test-activity-final-working.jpg');
    ...
    formData.append('image', fs.createReadStream(testImagePath), {
      filename: 'test-activity-final-working.jpg',
    ...
    const activityData = {
      titulo: 'Actividad de prueba final working con S3',
      participantes: [], // Array vacío para la prueba
      descripcion: 'Esta es una actividad de prueba final working para verificar la carga de imágenes a S3 desde la app móvil',
    ...
    console.log('\n🎉 [TEST ACTIVITY S3 FINAL WORKING] ¡Prueba completada exitosamente!');
    console.log('\n📋 Resumen:');
    ...
    console.log('   ✅ La app móvil puede cargar actividades con imágenes sin problemas');

  } catch (error) {
    console.error('\n❌ [TEST ACTIVITY S3 FINAL WORKING] Error:', error.message);
